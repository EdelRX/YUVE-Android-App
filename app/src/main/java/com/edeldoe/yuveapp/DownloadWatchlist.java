...
class DownloadWatchlist extends AsyncTask<String, String, String>{
            ...
            try {
                ...
                for (int i = 0; i < posterlist.size(); i++) { // Travel poster url list
                    ...
                    if (!path.equals("null")) { // If there is a url (sometimes there isn't)
                        ...
                        Bitmap bitmap = BitmapFactory.decodeResource(yuve_watchlist.activity.getResources(), R.drawable.posternotavailable2);
    ...
    public void onPostExecute(String result) {
        ...
        yuve_watchlist.activity.findViewById(R.id.progress_bar).setVisibility(View.INVISIBLE);
        gridView = (GridView) yuve_watchlist.activity.findViewById(R.id.gridView); // Buscamos la lista por ID y la guardamos aqui
        gridAdapter = new GridViewAdapter(yuve_watchlist.activity, R.layout.grid_item_layout, imageItems4); //Creamos un adaptador y le metemos las imagenes obtenidas en getData()
        ...
        if((yuve_start.myPref.getString("watchlist","noids")).equals("noids")) {
            Toast.makeText(yuve_watchlist.activity, "Watchlist is empty.", Toast.LENGTH_LONG).show();
        }

        if(imageItems4.isEmpty()){
            yuve_watchlist.clear_fav_button.setVisibility(View.INVISIBLE);
        }else{
            yuve_watchlist.clear_fav_button.setVisibility(View.VISIBLE);
        }
