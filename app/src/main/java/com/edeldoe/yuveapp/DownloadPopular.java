...
class DownloadPopular extends AsyncTask<String, String, String>{

    public static ArrayList<ImageItem> imageItems3 = new ArrayList<>(); //An array list to store images
    ...
    public static List titlelist3;
    public static List overviewlist3;
    public static List idlist3;
    ...
    protected String doInBackground(String... urls) {
        ...
        if(imageItems3.size() == 0) { // If we have no stored images
            ...
            try {
                ...
                titlelist3 = parsedata.readJsonStream(parseinput,2); // Calling method to obtain desired info
                overviewlist3 = parsedata.readJsonStream(parseinput,3); // Calling method to obtain desired info
                idlist3 = parsedata.readJsonStream(parseinput,4);  // Calling method to obtain desired info
                ...
                for (int i = 0; i < posterlist.size(); i++) { // Travel poster url list
                    ...
                    if (!path.equals("null")) { // If there is a url (sometimes there isn't)
                        ...
                        try {
                            ...
                            imageItems3.add(new ImageItem(bitmap)); // Add this downloaded poster to the image list

                        } finally {
                            //conn2.disconnect();
                        }
                    }else{
                        Bitmap bitmap = BitmapFactory.decodeResource(yuve_popular.activity.getResources(), R.drawable.posternotavailable2);
                        imageItems3.add(new ImageItem(bitmap));
                    }
    ...
    private String downloadUrl() throws IOException {
        ...
        String popularquery = yuve_start.ctx.getResources().getString(R.string.PopularURL)+ String.valueOf(page_p); // Query url to download JSON info

        try {
            URL url = new URL(popularquery); // Construct url object
    ...
    public void onPostExecute(String result) {
        ...
        yuve_popular.activity.findViewById(R.id.progress_bar).setVisibility(View.INVISIBLE);
        gridView = (GridView) yuve_popular.activity.findViewById(R.id.gridView); // Buscamos la lista por ID y la guardamos aqui
        gridView.setVisibility(View.VISIBLE);
        gridAdapter = new GridViewAdapter(yuve_popular.activity, R.layout.grid_item_layout, imageItems3); //Creamos un adaptador y le metemos las imagenes obtenidas en getData()
        ...
        Toast.makeText(yuve_popular.activity, "Long press for movie title. Tap for details.", Toast.LENGTH_LONG).show();
