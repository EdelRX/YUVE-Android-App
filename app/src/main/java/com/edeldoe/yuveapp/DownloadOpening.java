...
import com.edeldoe.yuveapp.R;

class DownloadOpening extends AsyncTask<String, String, String>{
            ...
            try {
                ...
                for (int i = 0; i < posterlist.size(); i++) { // Travel poster url list
                    ...
                    if (!path.equals("null")) { // If there is a url (sometimes there isn't)
                        ...
                        Bitmap bitmap = BitmapFactory.decodeResource(yuve_opening.activity.getResources(), R.drawable.posternotavailable2);
    ...
    private String downloadUrl() throws IOException {
        ...
        String openingquery = yuve_start.ctx.getResources().getString(R.string.OpeningURL)+String.valueOf(page); // Query url to download JSON info

        try {
            URL url = new URL(openingquery); // Construct url object
    ...
    public void onPostExecute(String result) {
        yuve_opening.activity.findViewById(R.id.progress_bar).setVisibility(View.INVISIBLE);
        gridView = (GridView) yuve_opening.activity.findViewById(R.id.gridView); // Buscamos la lista por ID y la guardamos aqui
        gridView.setVisibility(View.VISIBLE);
        gridAdapter = new GridViewAdapter(yuve_opening.activity, R.layout.grid_item_layout, imageItems); //Creamos un adaptador y le metemos las imagenes obtenidas en getData()
        ...
        Toast.makeText(yuve_opening.activity, "Long press for movie title. Tap for details.", Toast.LENGTH_LONG).show();
